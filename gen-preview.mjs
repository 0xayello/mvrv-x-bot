...
// Fetch real MVRV data from Coinmetrics
async function fetchMVRVData() {
  ...
  let url = `https://community-api.coinmetrics.io/v4/timeseries/asset-metrics?assets=btc&metrics=CapMVRVCur&start_time=${fmt(startDate)}&end_time=${fmt(endDate)}&pretty=true&page_size=10000`;

  console.log('Fetching MVRV history from Coinmetrics...');
  let allData = [];

  while (url) {
    const resp = await fetch(url, {
      headers: { 'User-Agent': 'mvrv-bot/2.0', 'Accept': 'application/json' }
    });
    if (!resp.ok) throw new Error(`API error: ${resp.status}`);
    const json = await resp.json();
    allData = [...allData, ...json.data];
    url = json.next_page_url || '';
    console.log(`  ${allData.length} points fetched...`);
  }
...
async function main() {
  const data = await fetchMVRVData();
  const lastValue = data.values[data.values.length - 1];
  console.log(`MVRV: ${lastValue.toFixed(2)}, Points: ${data.values.length}`);
  ...
  const yMin = 0;
  const yMax = 4;
  const getY = (value) => chartArea.bottom - ((value - yMin) / (yMax - yMin)) * chartArea.height;
  const getX = (index) => chartArea.left + (index / (data.values.length - 1)) * chartArea.width;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  writeFileSync('test-chart.png', Buffer.from(png));
